...
public class ProductController {
    ...
    /**
     * 分页显示商品 modelMap和model都是向页面传递数据的，类似于Request.setAttribute
     *
     * @param rows 每页显示多少条
     * @param page 当前页
     * @return
    ...
    public void getProductPage(Integer rows, Integer page,
                               HttpServletResponse response, ModelMap modelMap, ProductVo productVo) {
        // 封装查询条件
        if (productVo == null) {
        ...
        List<Product> productList = productService
                .getProductListWithPage(productVo);
        // 总条数
        ...
        try {
            ...
            for (String colorId : colorIds) {
                ...
                if (!StringUtils.isEmpty(colorId)) {
                    colors.add(colorService.getColorByKey(Integer
                            .valueOf(colorId)));
                }
            }
            ...
            // 根据商品no获取商品评论
            List<Comment> commentms = commentService
                    .getCommentsByProductNo(product.getNo());
            model.addAttribute("commentms", commentms);
