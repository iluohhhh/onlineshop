...
<script>
    function receiver(oid) {
        confirm("确定您已收到货？")
        window.location.href = "receiver.shtml?id=" + oid;
    }

    function comment(pno, oid) {
        window.location.href = "comment.shtml?id=" + oid + "&productNo=" + pno;
    }

    function viewProduct(pid) {
        window.location.href = "${base}/product/detail.shtml?id=" + pid;
    }
    ...
    <div class="h-title">
        <div class="h-logo">
            <a href="http://localhost:8080"><img
                    src="${base}/images/pic/logo.jpg"/></a>
        </div>
        <div class="h-search">
            <input type="text"/>
            <div class="h-se-btn">
...
<div class="w mt ofc">
    <div class="r wr profile">
        <h2 class="h2 h2_filter">
            <em style="color: red">全部订单</em>
        </h2>
            ...
            <c:forEach var="order" items="${orders}">
                <c:forEach items="${order.details}" var="detail">
                    <tr class="over">
                                    ...
                                    <dl>
                                        <dt>
                                            <a href="javascript:void(0)"
                                               title="${detail.product.name},${detail.product.no}"
                                               onclick="viewProduct(${detail.product.id})">${detail.product.name}</a>
                                        </dt>
                                    </dl>
                                </li>
                            </ul>
                        ...
                        <td><c:if test="${order.state <4 }">
                            <a href="javascript:void(0)" onclick="receiver(${order.id})">确认收货</a>
                        </c:if>
                            <c:if test="${order.state == 4 }">
                                <a href="javascript:void(0)"
                                   onclick="comment('${detail.product.no}',${order.id})">评论</a>
                            </c:if>
